...
void MenuBar::openBaseOptionTriggerred(){
    LEDMatrixPictureDataBase* db = new LEDMatrixPictureDataBase();
    if(!parent->get_parent()->get_dbMenager().openDataBase(*db)){
        SET_PTR_DO(db, nullptr);
    }
    parent->get_picturesListPanel().load(db);
}

void MenuBar::saveBaseOptionTriggerred(){
    LEDMatrixPictureDataBase* db = static_cast<LEDMatrixPictureDataBase*>(parent->get_picturesListPanel().get_list().get_currentElementRefObj());
